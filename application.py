...
from Services.EndpointProtectionService import NON_PROTECTED_ENDPOINTS, authRequired
...
if application.env == "production":
    ...
    def sendStaticComponent(requestedStaticDirectory, path):
        ...
        return send_from_directory(
            STATIC_ASSETS_DIRECTORY_ROOT + requestedStaticDirectory, path
        )
...
application.config.update(
    dict(MAIL_USERNAME=creds["MAIL_USER"], SENDGRID_KEY=creds["SENDGRID_API_KEY"])
)
...
api = Api(
    ...
    version="1.0",
    title="Agriworks API",
    description="All of the Controllers and their routes within Agriworks",
    doc="/swagger/",
)
