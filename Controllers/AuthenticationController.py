...
class Authorize(Resource):
    @auth_ns.doc(
        responses={200: "Success", 403: "Email already registered with our service"},
        params={
    ...
    def post(self):
        ...
        if user:
            if not user.password:
                ...
                ret.set_cookie(
                    "SID", str(session.sessionId), expires=session.dateExpires
                )
                return ret
...
class Logout(Resource):
    ...
    def post(self):
        try:
            ...
            return Response(
                "Unable to logout. Please reload and try again later.", status=400
            )
...
class Signup(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            return Response(
                "There's already an account with the provided email.", status=400
            )
        except:
            try:
...
class ConfirmUser(Resource):
    ...
    def post(self, userConfirmationId):
        try:
            ...
            return Response(
                "No account was found using the provided confirmation code.", status=404
            )
    ...
    def post(self, passwordResetId):
        try:
            ...
            if newPassword != confirmPassword:
                return Response(
                    "Password and Confirm Password fields must be the same", status=403
                )

            if AuthenticationService.resetPasswordSame(user, newPassword):
                return Response(
                    "Please choose a password that you haven't used before", status=403
                )
...
class VerifySession(Resource):
    ...
    def post(self):
        try:
            ...
            if not user:
                return Response(
                    "Your session has expired. Please login again.", status=401
                )
            else:
                return Response(user.email, status=200)
        except DoesNotExist as e:
            return Response(
                "Your session was not found. Please login again.", status=401
            )
    ...
    def post(self):
        try:
            ...
            session = AuthenticationService.getSession(
                SID
            )  # uses SID to get session from db
