...
class Account(Resource):
    @admin_ns.doc(
        ...
        params={
            ...
            "submit": {
                "in": "formData",
                "description": "either email or password to differentiate the forms",
                "required": True,
            },
    ...
    def post(self):
        ...
        if (
            form["submit"] == "email"
        ):  # might not actually be the way to do it, need to differeiante the forms

            formPassword = form["inputCurrentPassword"]  # password from form

            if (
                Authentication.saltPassword(formPassword) == sessionPassword
            ...
            if Authentication.saltPassword(formPassword) == sessionPassword:
                ...
                if formConfirmNewPassword == formNewPassword:
                    ...
                    return Response(
                        "Password does not match confirm password", status=400
                    )
